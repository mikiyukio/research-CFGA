...
for index in range(len(PARA)):
   ...
   ###################################################################################################################
    ...
    #################################################################################################################3
    # loaders = dataloader_msloss.get_dataloaders(train_paths, test_paths, train_labels, test_labels, args.img_size, 1, 1,
    #                                           1, SCDA_flag=1)  # 返回值为由可迭代DataLoader对象所组成的字典
    ...
    for phase in ['test']:
        ...
        for images, labels in loaders[phase]:  # 一个迭代器，迭代数据集中的每一batch_size图片;迭代的返回值dataset的子类中的__getitem__()方法是如何进行重写的；
            ...
            for flip in range(1):
                ...
                grad_cam_scda = PoolCam_yuhan_kernel_scda_origin_R101(net)  # 有最大连通域
