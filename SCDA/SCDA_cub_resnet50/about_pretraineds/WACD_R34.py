from SCDA_cub_resnet50.about_pretraineds.net_res50 import FGIAnet100_metric5_1, \
...
import cv2
from SCDA_cub_resnet50.about_pretraineds.grad_cam import PoolCam_yuhan_kernel_scda_origin, \
    PoolCam_yuhan_kernel_scda_origin_no_lcc, \
    PoolCam_yuhan_kernel_scda_origin_R18, \
    PoolCam_yuhan_kernel_scda_origin_R34, \
    PoolCam_yuhan_kernel_scda_origin_R101
...
for index in range(len(PARA)):
    ...
    save_model = torch.load(join(os.path.abspath(os.path.dirname(os.getcwd())), args.savedir,  'resnet34-333f7ec4.pth'))
    ...
    for phase in ['test']:
        ...
        for images, labels in loaders[phase]:  # 一个迭代器，迭代数据集中的每一batch_size图片;迭代的返回值dataset的子类中的__getitem__()方法是如何进行重写的；
            ...
            for flip in range(1):
                ...
                grad_cam_scda = PoolCam_yuhan_kernel_scda_origin_R34(net)  # 有最大连通域
                ...
                f4_1_1, output_mean_28,  f4_2_1, output_mean_26= output_mean[0], output_mean[1], \
    ...
    final_features['test'] = test_data
    # filename=join(os.path.abspath(os.path.dirname(os.getcwd())),'./datafile/layer4_conv2_weight.json')##'layer4.1.conv2.weight' + #'layer4.2.conv2.weight'
    filename = join(target_path, 'layer4_conv2_weight_R34.json')  # 'layer4.1.conv3.weight'
